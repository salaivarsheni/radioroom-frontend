...
const NewOrders = () => {
  ...
  const [freestoryList, setFreestoryList] = useState([]);
  const [showList, setShowList] = useState(false);

  useEffect(() => {
    const fetchFreeStories = async () => {
      try {
        ...
        if (Array.isArray(data?.data)) {
          ...
          count = data.data.length > 0 ? data.data[0]?.free_story_count : 0;
        } else if (data?.data && typeof data.data === 'object') {
          count = data.data.free_story_count || 0;
        } else if (data?.free_story_count !== undefined) {
  ...
  useEffect(() => {
    const fetchFreeStorieslist = async () => {
      try {
    ...
    <MainCard content={false}>
      <Box sx={{ p: 3, pb: 2 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <Button fullWidth variant="outlined" onClick={handleViewMore}>
              {showList ? 'Hide' : 'View more'}
            </Button>
            {showList && freestoryList?.free_data?.length > 0 && (
              <Box sx={{ mt: 2 }}>
                <table
                  style={{
                    width: '100%',
                ...
                >
                  ...
                  <tbody>
                    {freestoryList.free_data.map((item) => (
